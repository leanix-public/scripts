...
This script allows the user to delete every tag indicated in the import file.
...
# Delete the document
def deleteTag(id, header):
    """Query to delete a given tag.
...
with open(filename) as df:
    ...
    try: 
        for row in reader:
            deleteTag(row['id'], header)
    except Exception as e:
        logging.error(f'Error while processing tags: {e}')
